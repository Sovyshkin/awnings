...
const pageTitle = ref('Политика конфиденциальности')
const pageText = ref('')

async function loadContent() {
  const blocks = await fetchContentBlocks('privacy')
  const mainBlock = blocks.find((b) => b.block_type === 'legal' || (b.block_name || '').includes('Политика конфиденциальности'))
  if (mainBlock) {
